...
public interface FlashSaleDetailMapper {
	...
	// @Mapping(target = "product.evalues", source = "product.evalues") // Ánh xạ
	// evalues từ product vào Response_Product
	// @Mapping(target = "product.star", expression =
	// "java(product.getEvalues().stream().mapToDouble(Evalue::getStar).average().orElse(5.0))")
	// // Tính // toán // st // evalues
	// @Mapping(target = "product.quantityEvalue", expression =
	// "java(product.getEvalues().size()")
	// Response_FlashSaleDetail
	// flashSaleDetailToResponseFlashSaleDetail(FlashSaleDetail flashSaleDetail);

	@Mapping(target = "maxDiscount", ignore = true)
	@Mapping(target = "product.star", expression = "java(product.getEvalues().stream().mapToDouble(Evalue::getStar).average().orElse(0.0))") // Tính
																																				// toán
																																				// sao
																																				// cho
																																				// average
