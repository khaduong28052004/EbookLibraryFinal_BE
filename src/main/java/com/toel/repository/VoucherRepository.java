...
public interface VoucherRepository extends JpaRepository<Voucher, Integer> {
	...
	@Query("SELECT v FROM Voucher v WHERE v.account =?1 and v.dateStart < ?2 AND ?2< v.dateEnd AND v.isDelete = false ORDER BY v.sale ASC")
	List<Voucher> findAllByAccount(Account account, Date now);

	@Query("SELECT v FROM Voucher v WHERE (v.typeVoucher = ?1) AND (?2 BETWEEN v.dateStart AND v.dateEnd) AND (isDelete = false) ORDER BY v.sale ASC")
