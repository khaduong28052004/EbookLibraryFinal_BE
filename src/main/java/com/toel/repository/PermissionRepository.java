...
public interface PermissionRepository extends JpaRepository<Permission, Integer> {
    ...
    @Query("SELECT p FROM Permission p WHERE p.id NOT IN (SELECT rp.permission.id FROM RolePermission rp WHERE rp.role = ?1) and p.id IN (SELECT rp.permission.id FROM RolePermission rp where rp.role.name like 'ADMINV1')")
